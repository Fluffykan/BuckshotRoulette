...
Hopefully you clicked into this file before you started digging for wherever the hell "Main.java" is...
...
Then open the file called Main.java and hit the run button to run the java file, and everything should be good to go
...
About the game:
If Buckshot Roulette has somehow avoided you on Youtube / Twitch, or because you simply don't go on Youtube / Twitch, its a game where you try to kill the Computer aka. Dealer for 3 rounds.
...
So let's say you know the next round is a blank, obviously you won't want to shoot the Dealer with something that does no damage, so you will instead shoot yourself with a blank.
...
3. Handcuff : causes the Dealer to be unable to act (i.e. no shooting / using items) for 1 turn, BUT opponent will break free if you shoot it with a live round.
...
The Dealer knows what the next bullet is going to be, so it will ALWAYS make the best move (at least thats what I think it will do).
...
Clearing previous save data: only when you win the game i.e. beat the Dealer for 3 rounds, or if you choose to not load the previous game progress
